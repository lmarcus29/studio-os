...
import { LayoutDashboard, Users, FolderKanban, Package, Store, FileText, Calendar, CheckSquare, X, Trash2, Pencil, AlertCircle, Loader, Upload, FileImage, Download, Mail, TrendingUp, Folder, FolderOpen, Clock, MessageSquare, Menu, Settings, Wand2, LayoutGrid, List, Maximize2 } from 'lucide-react'
        ...
        <main style={{flex:1,overflowY:'auto',background:'#F7F3EE'}}>
          <div style={{padding: isMobile ? '1rem' : '1.5rem 2rem',flex:1}}>
            {dataLoading ? <LoadingSpinner /> : <>
              ...
              {detailType === 'project' && detailProject && <ProjectDetail project={detailProject} {...shared} studioProfile={studioProfile} onBack={() => { setDetailProject(null); setDetailType(detailClient ? 'client' : null) }} backLabel={detailClient ? 'Back to Client' : 'Back to Projects'} />}
              {!detailType && <>
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:'1.5rem',display:'flex',alignItems:'flex-end',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:"'DM Mono',monospace",fontSize:'0.6rem',color:'#C4B5A0',letterSpacing:'0.1em'}}>v12</span>
      </div>
...
function Projects({ projects, clients, reload, setDetailProject, showToast }) {
  const [search, setSearch] = useState('')
  ...
  async function handleSave(form, setLoading) {
    ...
    const { error } = modal === 'add'
      ? await supabase.from('projects').insert({ ...data, user_id })
      : await supabase.from('projects').update(data).eq('id', modal.id)
    if (error) { showToast('Could not save project: ' + error.message, 'error'); setLoading(false); return }
    setLoading(false)
    setModal(null)
    reload()
  }
  ...
  async function handleDelete() {
    ...
    if (error) { showToast('Could not delete project: ' + error.message, 'error'); setDeleteLoading(false); return }
    setDeleteLoading(false)
    setDeleteTarget(null)
    reload()
  }
...
function Vendors({ vendors, reload, showToast }) {
  const [search, setSearch] = useState('')
  const [modal, setModal] = useState(null)
  const [deleteTarget, setDeleteTarget] = useState(null)
  const [deleteLoading, setDeleteLoading] = useState(false)
  ...
  async function handleSave(form, setLoading) {
    ...
    const user_id = (await supabase.auth.getUser()).data.user.id
    const { error } = modal === 'add'
      ? await supabase.from('vendors').insert({ ...form, user_id })
      : await supabase.from('vendors').update(form).eq('id', modal.id)
    if (error) { showToast('Could not save vendor: ' + error.message, 'error'); setLoading(false); return }
    setLoading(false)
    setModal(null)
    reload()
  }
  ...
  async function handleDelete() {
    ...
    if (error) { showToast('Could not delete vendor: ' + error.message, 'error'); setDeleteLoading(false); return }
    setDeleteLoading(false)
    setDeleteTarget(null)
    reload()
  }
...
function Items({ items, projects, vendors, clients, reload, persistItemModal, setPersistItemModal, showToast }) {
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState('All')
  ...
  async function handleSave(form, setLoading) {
    ...
    const user_id = (await supabase.auth.getUser()).data.user.id
    const isNew = persistItemModal === 'add'
    const { error } = isNew
      ? await supabase.from('items').insert({ ...data, user_id })
      : await supabase.from('items').update(data).eq('id', persistItemModal.id)
    if (error) { showToast('Could not save item: ' + error.message, 'error'); setLoading(false); return }
    setLoading(false)
    setPersistItemModal(null)
    reload()
  }
  ...
  async function handleDelete() {
    ...
    if (error) { showToast('Could not delete item: ' + error.message, 'error'); setDeleteLoading(false); return }
    setDeleteLoading(false)
    setDeleteTarget(null)
    reload()
  }
        ...
        <table style={{width:'100%',fontSize:'0.82rem',borderCollapse:'collapse'}}>
          ...
          <tbody>
            ...
            {filtered.map(i => {
              ...
              return (
                <tr key={i.id} onClick={() => setPersistItemModal(i)}
                  style={{borderBottom:'1px solid rgba(42,37,32,0.04)',cursor:'pointer'}}
                  onMouseEnter={e => e.currentTarget.style.background='#F7F3EE'}
                  onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                  <td style={{padding:'0.5rem 0.75rem 0.5rem 1.25rem',width:52}} onClick={e => e.stopPropagation()}>
                    {i.image_url
                      ? <div style={{position:'relative',display:'inline-block',cursor:'zoom-in'}}
                          title="Click to view full image"
                          onClick={e => { e.stopPropagation(); window.open(i.image_url,'_blank') }}>
                          <img src={i.image_url} alt={i.name} style={{width:44,height:44,borderRadius:4,objectFit:'cover',border:'1px solid rgba(42,37,32,0.1)',display:'block'}}/>
                          <div style={{position:'absolute',inset:0,background:'rgba(42,37,32,0)',borderRadius:4,display:'flex',alignItems:'center',justifyContent:'center',transition:'background 0.15s'}}
                            onMouseEnter={e => e.currentTarget.style.background='rgba(42,37,32,0.35)'}
                            onMouseLeave={e => e.currentTarget.style.background='rgba(42,37,32,0)'}>
                            <Maximize2 size={12} style={{color:'white',opacity:0}} className="hover-show"/>
                          </div>
                        </div>
                      : <div style={{width:44,height:44,borderRadius:4,background:'#E8E0D5',display:'flex',alignItems:'center',justifyContent:'center'}}><Package size={16} style={{color:'#C4B5A0'}}/></div>
                    }
                  </td>
...
function Invoices({ invoices, clients, projects, items, payments, studioProfile, reload, showToast }) {
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState('All')
  const [modal, setModal] = useState(null)
  const [deleteTarget, setDeleteTarget] = useState(null)
  ...
  async function handleSave(form, setLoading) {
    ...
    let saveError = null
    if (isNew) {
      const { error } = await supabase.from('invoices').insert({ ...data, user_id })
      saveError = error
    } else {
      const { error } = await supabase.from('invoices').update(data).eq('id', modal.id)
      saveError = error
    }

    if (saveError) {
      showToast('Could not save invoice: ' + saveError.message, 'error')
      setLoading(false)
      return
    }
    ...
    // Auto-log invoice creation to activity
    if (isNew && form.client_id) {
      const dueStr = form.due ? new Date(form.due + 'T00:00:00').toLocaleDateString('en-US', {month:'short',day:'numeric',year:'numeric'}) : 'no due date'
      await supabase.from('activity_log').insert({
        user_id, client_id: form.client_id,
        type: 'invoice', classification: 'Invoice',
        subject: `Invoice ${form.num || ''} created — $${(Number(form.amount)||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`,
        body: `Due: ${dueStr} · Status: ${form.status || 'Pending'}`
      })
    }
...
async function handleDelete() {
  ...
  async function generatePDF(invoice) {
    ...
    // ── LOGO: fetch as base64 if available ───────────────────
    let logoBase64 = null
    let logoFormat = 'PNG'
    if (brand.logoUrl) {
      try {
        ...
        const ext = brand.logoUrl.split('.').pop().toLowerCase()
    ...
    if (logoBase64) {
      // Logo image — max 40px tall in header
      try {
        doc.addImage(logoBase64, logoFormat, margin + 4, 6, 0, 18)
      } catch(e) {
    ...
    // Always show balance/paid status pill
    const isPaid = remainingBalance <= 0 || invoice.status === 'Paid'
    const isPartial = totalPaidAmt > 0 && remainingBalance > 0
    const pillColor = isPaid ? [107,124,110] : invoice.status === 'Overdue' ? [196,98,45] : [42,37,32]
    doc.setFillColor(...pillColor)
    doc.roundedRect(totalsX - 4, y - 5, pageW - margin - totalsX + 4 + margin, 12, 2, 2, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(10)
    doc.setFont('helvetica', 'bold')
    const pillLabel = isPaid ? 'PAID IN FULL' : isPartial ? 'BALANCE DUE' : invoice.status === 'Overdue' ? 'OVERDUE — BALANCE DUE' : 'BALANCE DUE'
    doc.text(pillLabel, totalsX, y + 3)
    doc.text(`$${remainingBalance.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`, pageW - margin, y + 3, { align: 'right' })
    y += 18
                      ...
                      <td colSpan={9} style={{padding:'0 1.25rem 1.25rem'}}>
                        <div style={{background:'#F7F3EE',borderRadius:6,padding:'1rem'}}>
                          ...
                          <PaymentForm invoiceId={i.id} invoiceAmount={Number(i.amount||0)} totalPaid={totalPaid} onSave={async (form) => {
                            ...
                            const nowPaidInFull = newPaid >= Number(i.amount||0)
                            if (nowPaidInFull) {
                            ...
                            if (i.client_id) {
                              ...
                              await supabase.from('activity_log').insert({
                                ...
                                subject: `Payment of $${Number(form.amount).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})} logged`,
                                body: `Invoice: ${i.num} · Method: ${form.method || 'Not specified'}${nowPaidInFull ? ' · PAID IN FULL' : ''}`
                              })
                            }
                            reload()
                          }} />
...
function Tasks({ tasks, projects, clients, reload, showToast }) {
  const [modal, setModal] = useState(null)
  const [deleteTarget, setDeleteTarget] = useState(null)
  const [deleteLoading, setDeleteLoading] = useState(false)
  ...
  async function handleSave(form, setLoading) {
    ...
    const { error } = modal === 'add'
      ? await supabase.from('tasks').insert({ ...taskData, user_id })
      : await supabase.from('tasks').update(taskData).eq('id', modal.id)
    if (error) { showToast('Could not save task: ' + error.message, 'error'); setLoading(false); return }
    setLoading(false)
    setModal(null)
    reload()
  }
  ...
  async function handleDelete() {
    ...
    if (error) { showToast('Could not delete task: ' + error.message, 'error'); setDeleteLoading(false); return }
    setDeleteLoading(false)
    setDeleteTarget(null)
    reload()
  }
...
function Files({ projects, clients, fileMetadata, reload }) {
  ...
  async function loadFiles() {
    ...
    // Build enriched list from storage files
    const enriched = await Promise.all(storageFiles.map(async f => {
    ...
    // Also include orphan metadata records (no matching storage file) so they can be deleted
    const storagePaths = enriched.map(f => f.storagePath)
    const orphanMeta = fileMetadata.filter(m =>
      !storagePaths.includes(m.storage_path) &&
      m.storage_path.startsWith(user.id + '/')
    )
    const orphanEntries = orphanMeta.map(m => ({
      name: m.display_name || m.storage_path.split('/').pop(),
      signedUrl: null,
      storagePath: m.storage_path,
      projectId: m.project_id || '',
      clientId: m.client_id || '',
      roomType: m.room_type || '',
      styleTag: m.style_tag || '',
      fileType: m.file_type || '',
      fileUrl: m.url || '',
      displayName: m.display_name || m.storage_path.split('/').pop(),
      metaId: m.id,
      isOrphan: true,
    }))

    setFiles([...enriched, ...orphanEntries])
    setLoading(false)
  }
  ...
  async function handleUpload(e) {
    ...
    if (!error) {
      ...
      await supabase.from('file_metadata').insert({
        user_id: u2.id, storage_path: path, display_name: file.name,
        project_id: uploadTags.project_id || null, client_id: uploadTags.client_id || null,
        room_type: uploadTags.room_type || null, style_tag: uploadTags.style_tag || null,
        file_type: uploadTags.file_type || null, url: uploadTags.url || null,
      })
      // Auto-log to activity if linked to a client
      if (uploadTags.client_id) {
        await supabase.from('activity_log').insert({
          user_id: u2.id, client_id: uploadTags.client_id,
          type: 'file', classification: 'File Upload',
          subject: `File uploaded: ${file.name}`,
          body: [uploadTags.project_id ? `Project: ${projects.find(p=>p.id===uploadTags.project_id)?.name||''}` : '', uploadTags.room_type || '', uploadTags.file_type || ''].filter(Boolean).join(' · ')
        })
      }
  ...
  async function handleDelete() {
    setDeleteLoading(true)
    // 1. Try storage delete — ignore error if file doesn't exist
    try {
      await supabase.storage.from('studio-files').remove([deleteTarget.storagePath])
    } catch(e) { /* orphan — storage file already gone */ }
    // 2. Delete metadata record if it exists
    if (deleteTarget.metaId) {
      await supabase.from('file_metadata').delete().eq('id', deleteTarget.metaId)
    }
    // 3. Also catch any orphan metadata by storage_path (belt and suspenders)
    if (deleteTarget.storagePath) {
      await supabase.from('file_metadata').delete().eq('storage_path', deleteTarget.storagePath)
    }
    // 4. Remove from local state immediately so UI updates even if reload is slow
    setFiles(prev => prev.filter(f => f.storagePath !== deleteTarget.storagePath))
    setDeleteLoading(false)
    setDeleteTarget(null)
    reload()
  }
  ...
  const filtered = files.filter(f => {
    ...
    const matchRoom = !roomFilter || f.roomType === roomFilter
    const matchStyle = !styleFilter || f.styleTag === styleFilter
    return matchSearch && matchProject && matchClient && matchFileType && matchRoom && matchStyle
  })

  const imageFiles = filtered.filter(f => isImage(f.name || f.displayName))
  const galleryFiles = viewMode === 'gallery' ? imageFiles : filtered
    ...
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem'}}>
        ...
        <div style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>
          {/* View mode toggle */}
          <div style={{display:'flex',border:'1px solid rgba(42,37,32,0.15)',borderRadius:4,overflow:'hidden'}}>
            <button onClick={() => setViewMode('grid')} title="Grid view"
              style={{padding:'0.4rem 0.6rem',background:viewMode==='grid'?'#2A2520':'transparent',border:'none',cursor:'pointer',display:'flex',alignItems:'center',color:viewMode==='grid'?'white':'#8A8278'}}>
              <List size={14}/>
            </button>
            <button onClick={() => setViewMode('gallery')} title="Image gallery"
              style={{padding:'0.4rem 0.6rem',background:viewMode==='gallery'?'#2A2520':'transparent',border:'none',cursor:'pointer',display:'flex',alignItems:'center',color:viewMode==='gallery'?'white':'#8A8278'}}>
              <LayoutGrid size={14}/>
            </button>
          </div>
          <button onClick={() => setShowUploadPanel(!showUploadPanel)}
            style={{background:'#C4622D',color:'white',padding:'0.5rem 1.1rem',borderRadius:4,fontSize:'0.78rem',fontWeight:500,border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:'0.4rem'}}>
      ...
      {viewMode === 'gallery' && (
        <div style={{background:'#F5EDD8',border:'1px solid rgba(184,150,62,0.2)',borderRadius:6,padding:'0.6rem 1rem',marginBottom:'1rem',fontSize:'0.78rem',color:'#B8963E',display:'flex',alignItems:'center',gap:'0.5rem'}}>
          <LayoutGrid size={13}/> Image Gallery — showing {imageFiles.length} image{imageFiles.length!==1?'s':''} · Click any image to expand
        </div>
      )}
      ...
      <div style={{display:'flex',gap:'0.5rem',marginBottom:'1.25rem',flexWrap:'wrap'}}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search files…" className={inputClass} style={{...inputStyle,width:180}} />
        ...
        <select value={roomFilter} onChange={e => setRoomFilter(e.target.value)} className={inputClass} style={{...inputStyle,width:'auto'}}>
          <option value="">All Rooms</option>
          {ROOM_TYPES.map(t => <option key={t}>{t}</option>)}
        </select>
        <select value={styleFilter} onChange={e => setStyleFilter(e.target.value)} className={inputClass} style={{...inputStyle,width:'auto'}}>
      ...
      {loading ? <LoadingSpinner /> : viewMode === 'gallery' ? (
        // ── GALLERY MODE ──────────────────────────────────────
        <div>
          {imageFiles.length === 0 ? (
            <div style={{textAlign:'center',padding:'4rem 0',color:'#8A8278',fontSize:'0.82rem'}}>No images found — upload mood boards or photos to see them here</div>
          ) : (
            <div style={{columns:'4 200px',gap:'0.75rem'}}>
              {imageFiles.map(f => (
                <div key={f.name} onClick={() => setLightboxFile(f)}
                  style={{breakInside:'avoid',marginBottom:'0.75rem',borderRadius:6,overflow:'hidden',cursor:'zoom-in',position:'relative',border:'1px solid rgba(42,37,32,0.08)'}}>
                  <img src={f.signedUrl || f.fileUrl} alt={f.displayName}
                    style={{width:'100%',display:'block',borderRadius:6}}/>
                  <div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(42,37,32,0.7))',padding:'1.5rem 0.5rem 0.5rem',borderRadius:'0 0 6px 6px',opacity:0,transition:'opacity 0.2s'}}
                    onMouseEnter={e => e.currentTarget.style.opacity='1'}
                    onMouseLeave={e => e.currentTarget.style.opacity='0'}>
                    <p style={{color:'white',fontSize:'0.72rem',fontWeight:500,margin:0,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{f.displayName}</p>
                    <div style={{display:'flex',gap:'0.25rem',marginTop:'0.25rem',flexWrap:'wrap'}}>
                      {f.roomType && <span style={{background:'rgba(107,124,110,0.8)',color:'white',fontSize:'0.55rem',padding:'0.1rem 0.3rem',borderRadius:4}}>{f.roomType}</span>}
                      {f.styleTag && <span style={{background:'rgba(184,150,62,0.8)',color:'white',fontSize:'0.55rem',padding:'0.1rem 0.3rem',borderRadius:4}}>{f.styleTag}</span>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem'}}>
          ...
          {filtered.map(f => (
            <div key={f.name || f.metaId} style={{background:'#FDFAF6',border:`1px solid ${f.isOrphan ? 'rgba(196,98,45,0.3)' : 'rgba(42,37,32,0.1)'}`,borderRadius:8,overflow:'hidden'}}>
              <div style={{height:140,background: f.isOrphan ? '#F5E6DE' : '#E8E0D5',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',cursor: isImage(f.name||f.displayName) ? 'zoom-in' : 'default'}}
                onClick={() => isImage(f.name||f.displayName) && setLightboxFile(f)}>
                {isImage(f.name||f.displayName) && f.signedUrl
                  ? <img src={f.signedUrl} alt={f.displayName} style={{width:'100%',height:'100%',objectFit:'cover'}} />
                  : <FileImage size={36} style={{color: f.isOrphan ? '#C4622D' : '#C4B5A0'}} />
                }
              </div>
              <div style={{padding:'0.75rem'}}>
                ...
                {f.isOrphan
                  ? <p style={{fontSize:'0.68rem',color:'#C4622D',marginTop:2}}>⚠️ File missing from storage — delete to clean up</p>
                ...
                <div style={{display:'flex',gap:'0.5rem',marginTop:'0.5rem',alignItems:'center'}}>
                  {f.signedUrl && <a href={f.signedUrl} target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',gap:3,fontSize:'0.72rem',color:'#C4622D',textDecoration:'none'}}><Download size={12}/> View</a>}
                  {f.fileUrl && <a href={f.fileUrl} target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',gap:3,fontSize:'0.72rem',color:'#6B7C6E',textDecoration:'none'}}>🔗 Product</a>}
                  <button onClick={() => setDeleteTarget(f)} style={{display:'flex',alignItems:'center',gap:3,fontSize:'0.72rem',color: f.isOrphan ? '#C4622D' : '#C4B5A0',background:'none',border:'none',cursor:'pointer',marginLeft:'auto',fontWeight: f.isOrphan ? 500 : 400}}><Trash2 size={12}/> Delete</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* Lightbox */}
      {lightboxFile && (
        <div style={{position:'fixed',inset:0,background:'rgba(26,20,16,0.93)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300,padding:'2rem'}}
          onClick={() => setLightboxFile(null)}>
          <button onClick={() => setLightboxFile(null)} style={{position:'absolute',top:'1.5rem',right:'1.5rem',background:'rgba(255,255,255,0.1)',border:'none',cursor:'pointer',color:'white',borderRadius:'50%',width:40,height:40,display:'flex',alignItems:'center',justifyContent:'center'}}><X size={20}/></button>
          <div onClick={e => e.stopPropagation()} style={{maxWidth:'85vw',maxHeight:'85vh',display:'flex',flexDirection:'column',gap:'0.75rem',alignItems:'center'}}>
            <img src={lightboxFile.signedUrl || lightboxFile.fileUrl} alt={lightboxFile.displayName}
              style={{maxWidth:'100%',maxHeight:'80vh',objectFit:'contain',borderRadius:4,boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}/>
            <div style={{textAlign:'center'}}>
              <p style={{color:'white',fontSize:'0.9rem',fontWeight:500}}>{lightboxFile.displayName}</p>
              <div style={{display:'flex',gap:'0.5rem',justifyContent:'center',marginTop:'0.4rem',flexWrap:'wrap'}}>
                {lightboxFile.roomType && <span style={{background:'rgba(107,124,110,0.5)',color:'white',fontSize:'0.7rem',padding:'0.15rem 0.5rem',borderRadius:8}}>{lightboxFile.roomType}</span>}
                {lightboxFile.styleTag && <span style={{background:'rgba(184,150,62,0.5)',color:'white',fontSize:'0.7rem',padding:'0.15rem 0.5rem',borderRadius:8}}>{lightboxFile.styleTag}</span>}
                {lightboxFile.clientId && <span style={{background:'rgba(196,98,45,0.5)',color:'white',fontSize:'0.7rem',padding:'0.15rem 0.5rem',borderRadius:8}}>{clients.find(c=>c.id===lightboxFile.clientId)?.name||''}</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      {deleteTarget && <ConfirmDeleteModal name={deleteTarget.displayName} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} loading={deleteLoading} />}
    </div>
  )
}
...
function ClientDetail({ client, projects, invoices, tasks, events, payments, clients, fileMetadata, items, reload, onBack, setDetailProject, studioProfile, showToast, logClientActivity }) {
  ...
  const [showAIProcurement, setShowAIProcurement] = useState(false)
  ...
  return (
    <div>
      ...
      <div style={{background:'#FDFAF6',border:'1px solid rgba(42,37,32,0.1)',borderRadius:8,padding:'1.5rem',marginBottom:'1.25rem'}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'1rem'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:'0.75rem'}}>
            ...
            <button onClick={() => setShowAIEmail(true)} style={{background:'#B8963E',color:'white',border:'none',borderRadius:4,padding:'0.35rem 0.75rem',fontSize:'0.75rem',cursor:'pointer',display:'flex',alignItems:'center',gap:'0.3rem'}}>
              <Wand2 size={12}/> AI Email
            </button>
            <button onClick={() => setShowAIBrief(true)} style={{background:'#6B7C6E',color:'white',border:'none',borderRadius:4,padding:'0.35rem 0.75rem',fontSize:'0.75rem',cursor:'pointer',display:'flex',alignItems:'center',gap:'0.3rem'}}>
              <Wand2 size={12}/> AI Brief
            </button>
            <button onClick={() => setShowMessaging(true)} style={{background:'#4A4540',color:'white',border:'none',borderRadius:4,padding:'0.35rem 0.75rem',fontSize:'0.75rem',cursor:'pointer',display:'flex',alignItems:'center',gap:'0.3rem'}}>
              ✉️ Message
            </button>
      ...
      {showAIEmail && <AIEmailComposer client={client} studioProfile={studioProfile} onClose={() => setShowAIEmail(false)}
        onUseEmail={(text) => { setShowAIEmail(false); setShowMessaging(true) }} />}
      {showAIBrief && <AIProjectBrief client={client} projects={projects} studioProfile={studioProfile} onClose={() => setShowAIBrief(false)} />}
      {showAIProcurement && <AIProcurementAssistant client={client} project={null} studioProfile={studioProfile} onClose={() => setShowAIProcurement(false)} />}
    </div>
  )
}

function ProjectDetail({ project, clients, projects, vendors, items, tasks, invoices, events, payments, timeLogs, fileMetadata, studioProfile, reload, onBack, backLabel }) {
  ...
  return (
    <div>
      ...
      <div style={{background:'#FDFAF6',border:'1px solid rgba(42,37,32,0.1)',borderRadius:8,padding:'1.5rem',marginBottom:'1.25rem'}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'1rem'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:'0.75rem'}}>
            ...
            <button onClick={() => setShowAIProcurement(true)} style={{background:'#B8963E',color:'white',border:'none',borderRadius:4,padding:'0.35rem 0.75rem',fontSize:'0.75rem',cursor:'pointer',display:'flex',alignItems:'center',gap:'0.3rem'}}>
              <Wand2 size={12}/> AI Sourcing
            </button>
      ...
      {showAIProcurement && <AIProcurementAssistant client={client} project={project} studioProfile={studioProfile} onClose={() => setShowAIProcurement(false)} />}
    </div>
  )
}
...
function ClientFilePanel({ client, project, projects, fileMetadata, reload }) {
  ...
  useEffect(() => {
    async function loadSigned() {
      const enriched = await Promise.all(relevantFiles.map(async f => {
        const { data } = await supabase.storage.from('studio-files').createSignedUrl(f.storage_path, 3600)
  ...
  function isImage(path) { return /\.(jpg|jpeg|png|gif|webp)$/i.test(path) }

  const filtered = signedFiles.filter(f => {
    const matchRoom = !roomFilter || f.room_type === roomFilter
    const matchStyle = !styleFilter || f.style_tag === styleFilter
    return matchRoom && matchStyle
  })

  const imageFiles = filtered.filter(f => isImage(f.storage_path))
  ...
  async function handleUpload(e) {
    ...
    if (!error) {
      await supabase.from('file_metadata').insert({
        user_id: user.id, storage_path: path, display_name: file.name,
        project_id: uploadTags.project_id || null, client_id: uploadTags.client_id || null,
        room_type: uploadTags.room_type || null, style_tag: uploadTags.style_tag || null,
        file_type: uploadTags.file_type || null, url: uploadTags.url || null,
      })
  ...
  async function handleDelete(f) {
    try { await supabase.storage.from('studio-files').remove([f.storage_path]) } catch(e) {}
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem',flexWrap:'wrap',gap:'0.5rem'}}>
        <div style={{display:'flex',alignItems:'center',gap:'0.75rem'}}>
          <span style={{fontFamily:"'DM Mono',monospace",fontSize:'0.58rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'#8A8278'}}>
            {filtered.length} file{filtered.length!==1?'s':''}
            {imageFiles.length > 0 && ` · ${imageFiles.length} image${imageFiles.length!==1?'s':''}`}
          </span>
          <div style={{display:'flex',border:'1px solid rgba(42,37,32,0.15)',borderRadius:4,overflow:'hidden'}}>
            <button onClick={() => setViewMode('grid')} title="Grid view"
              style={{padding:'0.3rem 0.5rem',background:viewMode==='grid'?'#2A2520':'transparent',border:'none',cursor:'pointer',display:'flex',alignItems:'center',color:viewMode==='grid'?'white':'#8A8278'}}>
              <List size={13}/>
            </button>
            <button onClick={() => setViewMode('gallery')} title="Image gallery"
              style={{padding:'0.3rem 0.5rem',background:viewMode==='gallery'?'#2A2520':'transparent',border:'none',cursor:'pointer',display:'flex',alignItems:'center',color:viewMode==='gallery'?'white':'#8A8278'}}>
              <LayoutGrid size={13}/>
            </button>
          </div>
        </div>
        <div style={{display:'flex',gap:'0.4rem',alignItems:'center',flexWrap:'wrap'}}>
          <select value={roomFilter} onChange={e => setRoomFilter(e.target.value)} className={inputClass} style={{...inputStyle,fontSize:'0.72rem',padding:'0.3rem 0.5rem',width:'auto'}}>
            <option value="">All Rooms</option>
            {ROOM_TYPES.map(t => <option key={t}>{t}</option>)}
          </select>
          <select value={styleFilter} onChange={e => setStyleFilter(e.target.value)} className={inputClass} style={{...inputStyle,fontSize:'0.72rem',padding:'0.3rem 0.5rem',width:'auto'}}>
            <option value="">All Styles</option>
            {STYLE_TAGS.map(t => <option key={t}>{t}</option>)}
          </select>
          <button onClick={() => setShowUpload(!showUpload)}
            style={{background:'#C4622D',color:'white',padding:'0.4rem 0.9rem',borderRadius:4,fontSize:'0.75rem',fontWeight:500,border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:'0.4rem'}}>
      ...
      {filtered.length === 0
        ...
        : viewMode === 'gallery'
        ? (
          <div style={{columns:'3 160px',gap:'0.6rem'}}>
            {imageFiles.length === 0
              ? <p style={{color:'#8A8278',fontSize:'0.82rem',padding:'1rem 0'}}>No images — switch to grid view to see all files</p>
              : imageFiles.map(f => (
                <div key={f.id} onClick={() => setLightboxFile(f)}
                  style={{breakInside:'avoid',marginBottom:'0.6rem',borderRadius:6,overflow:'hidden',cursor:'zoom-in',position:'relative'}}>
                  <img src={f.signedUrl} alt={f.display_name} style={{width:'100%',display:'block',borderRadius:6}}/>
                  <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'1rem 0.4rem 0.4rem',background:'linear-gradient(transparent,rgba(42,37,32,0.65))',borderRadius:'0 0 6px 6px'}}>
                    <p style={{color:'white',fontSize:'0.65rem',margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.display_name}</p>
                  </div>
                </div>
              ))
            }
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'0.75rem'}}>
            {filtered.map(f => (
              <div key={f.id} style={{background:'#FDFAF6',border:'1px solid rgba(42,37,32,0.1)',borderRadius:8,overflow:'hidden'}}>
                <div style={{height:100,background:'#E8E0D5',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',cursor:isImage(f.storage_path)?'zoom-in':'default'}}
                  onClick={() => isImage(f.storage_path) && setLightboxFile(f)}>
                  {isImage(f.storage_path) && f.signedUrl
                    ? <img src={f.signedUrl} alt={f.display_name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                    : <FileImage size={28} style={{color:'#C4B5A0'}}/>
                  }
                </div>
                <div style={{padding:'0.5rem'}}>
                  <p style={{fontSize:'0.75rem',fontWeight:500,color:'#2A2520',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.display_name}</p>
                  <div style={{display:'flex',flexWrap:'wrap',gap:'0.2rem',marginTop:'0.25rem'}}>
                    {f.file_type && <span style={{background:'#F5EDD8',color:'#B8963E',fontSize:'0.58rem',fontFamily:"'DM Mono',monospace",padding:'0.1rem 0.35rem',borderRadius:8}}>{f.file_type}</span>}
                    {f.room_type && <span style={{background:'#EBF0EC',color:'#6B7C6E',fontSize:'0.58rem',fontFamily:"'DM Mono',monospace",padding:'0.1rem 0.35rem',borderRadius:8}}>{f.room_type}</span>}
                  </div>
                  <div style={{display:'flex',gap:'0.4rem',marginTop:'0.35rem',alignItems:'center'}}>
                    {f.signedUrl && <a href={f.signedUrl} target="_blank" rel="noopener noreferrer" style={{fontSize:'0.68rem',color:'#C4622D',textDecoration:'none',display:'flex',alignItems:'center',gap:2}}><Download size={10}/> View</a>}
                    {f.url && <a href={f.url} target="_blank" rel="noopener noreferrer" style={{fontSize:'0.68rem',color:'#6B7C6E',textDecoration:'none'}}>🔗</a>}
                    <button onClick={() => handleDelete(f)} style={{marginLeft:'auto',color:'#C4B5A0',background:'none',border:'none',cursor:'pointer',display:'flex'}}><Trash2 size={11}/></button>
                  </div>
                </div>
              </div>
            ))}
      ...
      {lightboxFile && (
        <div style={{position:'fixed',inset:0,background:'rgba(26,20,16,0.93)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300,padding:'2rem'}}
          ...
          <button onClick={() => setLightboxFile(null)} style={{position:'absolute',top:'1.5rem',right:'1.5rem',background:'rgba(255,255,255,0.1)',border:'none',cursor:'pointer',color:'white',borderRadius:'50%',width:40,height:40,display:'flex',alignItems:'center',justifyContent:'center'}}><X size={20}/></button>
          <div onClick={e => e.stopPropagation()} style={{maxWidth:'85vw',maxHeight:'85vh',display:'flex',flexDirection:'column',gap:'0.75rem',alignItems:'center'}}>
            <img src={lightboxFile.signedUrl} alt={lightboxFile.display_name}
              style={{maxWidth:'100%',maxHeight:'80vh',objectFit:'contain',borderRadius:4,boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}/>
            <p style={{color:'white',fontSize:'0.9rem',fontWeight:500,margin:0}}>{lightboxFile.display_name}</p>
          </div>
        </div>
      )}
    </div>
...
function InvoicePanel({ invoices, payments, projects, clients, items, studioProfile, reload }) {
  ...
  async function generatePanelPDF(invoice) {
    const doc = new jsPDF()
    const client = clients.find(c => c.id === invoice.client_id)
    const project = projects.find(p => p.id === invoice.project_id)
    const sp = studioProfile || {}
    ...
    if (logoBase64) {
      try { doc.addImage(logoBase64, logoFormat, margin+4, 6, 0, 18) } catch(e) {
        doc.setTextColor(...ink); doc.setFontSize(22); doc.setFont('helvetica','bold')
        doc.text(brand.name.toUpperCase(),margin+4,20)
      }
      doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(...muted)
    ...
    const invPaidTotal = invPayments.reduce((s,p)=>s+(Number(p.amount)||0),0)
    const remaining = Math.max(0,total-invPaidTotal)
    ...
    // Always show balance/paid pill
    const isPaidP = remaining<=0 || invoice.status==='Paid'
    const isPartialP = invPaidTotal>0 && remaining>0
    const pillColorP = isPaidP?[107,124,110]:invoice.status==='Overdue'?[196,98,45]:[42,37,32]
    doc.setFillColor(...pillColorP)
    doc.roundedRect(totalsX-4,y-5,pageW-margin-totalsX+4+margin,12,2,2,'F')
    doc.setTextColor(255,255,255); doc.setFontSize(10); doc.setFont('helvetica','bold')
    const pillLabelP = isPaidP?'PAID IN FULL':isPartialP?'BALANCE DUE':invoice.status==='Overdue'?'OVERDUE — BALANCE DUE':'BALANCE DUE'
    doc.text(pillLabelP,totalsX,y+3)
    doc.text(`$${remaining.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`,pageW-margin,y+3,{align:'right'})
    y+=18
    if(invoice.notes){
      doc.setDrawColor(...bark); doc.setLineWidth(0.3); doc.line(margin,y,pageW-margin,y); y+=6
      doc.setFontSize(7.5); doc.setFont('helvetica','bold'); doc.setTextColor(...terra)
...
function StudioSettings({ studioProfile, reload, showToast }) {
  ...
  async function handleRestartTour() {
    ...
    if (studioProfile?.id) {
      await supabase.from('studio_profile').update({ onboarding_complete: false }).eq('id', studioProfile.id)
      reload()
      showToast('Onboarding tour will appear on next page load', 'success')
    }
  ...
  const studioName = form.studio_name || 'Your Studio'
  const thankYou = form.thank_you_note || 'Thank you for the privilege of designing your space.'
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1.5rem',maxWidth:860}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          <button onClick={() => setShowPortalPreview(true)}
            style={{background:'transparent',color:'#6B7C6E',padding:'0.75rem',borderRadius:4,fontSize:'0.85rem',fontWeight:500,border:'1px solid #6B7C6E',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.5rem'}}>
            👁 Preview Client Email
          </button>

          {/* Restart Tour */}
          <button onClick={handleRestartTour} disabled={restartingTour || !studioProfile?.id}
            style={{background:'transparent',color:'#8A8278',padding:'0.65rem',borderRadius:4,fontSize:'0.8rem',border:'1px solid rgba(42,37,32,0.15)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.5rem'}}>
            {restartingTour ? <><Loader size={13} className="animate-spin" /> Resetting…</> : '↺ Restart Onboarding Tour'}
          </button>
      ...
      {showPortalPreview && (
        <div style={{position:'fixed',inset:0,background:'rgba(42,37,32,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:'1rem',overflowY:'auto'}}>
          <div style={{background:'#F7F3EE',borderRadius:8,boxShadow:'0 20px 60px rgba(42,37,32,0.25)',width:'100%',maxWidth:600,maxHeight:'90vh',overflowY:'auto'}}>
            {/* Modal header */}
            <div style={{background:'#2A2520',padding:'1rem 1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',borderRadius:'8px 8px 0 0'}}>
              <div>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'0.6rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'#B8963E',marginBottom:'0.2rem'}}>Preview Mode</div>
                <div style={{fontSize:'0.85rem',color:'#C4B5A0',fontFamily:"'DM Sans',sans-serif"}}>This is what your client sees in their inbox</div>
              </div>
              <button onClick={() => setShowPortalPreview(false)} style={{background:'none',border:'none',cursor:'pointer',color:'#8A8278'}}><X size={18}/></button>
            </div>

            {/* Simulated email client chrome */}
            <div style={{background:'#ECEEF0',padding:'0.75rem 1rem',borderBottom:'1px solid #D0D3D7'}}>
              <div style={{fontSize:'0.78rem',color:'#444',marginBottom:'0.25rem'}}><span style={{color:'#888',marginRight:'0.5rem'}}>From:</span>{studioName} &lt;studio@decoratoros.com&gt;</div>
              <div style={{fontSize:'0.78rem',color:'#444',marginBottom:'0.25rem'}}><span style={{color:'#888',marginRight:'0.5rem'}}>To:</span>Sarah Morrison &lt;sarah@morrisonhome.com&gt;</div>
              <div style={{fontSize:'0.85rem',fontWeight:600,color:'#222'}}>Invoice Reminder: INV-2041 — $4,300.00 due April 15, 2026</div>
            </div>

            {/* Actual email HTML preview */}
            <div style={{padding:'1.5rem',background:'#F7F3EE'}}>
              {/* Email content */}
              <div style={{fontFamily:'Georgia,serif',maxWidth:'100%',margin:'0 auto',background:'#F7F3EE'}}>
                {/* Header */}
                <div style={{background:'#2A2520',padding:'28px 32px',borderLeft:'4px solid #B8963E'}}>
                  {form.logo_url
                    ? <img src={form.logo_url} alt={studioName} style={{maxHeight:40,maxWidth:180,objectFit:'contain',marginBottom:8,display:'block'}} />
                    : null
                  }
                  <h1 style={{color:'#F7F3EE',margin:0,fontSize:22,fontWeight:400,letterSpacing:'0.05em'}}>{studioName}</h1>
                  <p style={{color:'#C4B5A0',margin:'6px 0 0',fontFamily:'monospace',fontSize:11,letterSpacing:'0.15em',textTransform:'uppercase'}}>Invoice Reminder</p>
                </div>
                {/* Body */}
                <div style={{background:'#FDFAF6',padding:32,border:'1px solid #E8E0D5',borderTop:'none'}}>
                  <p style={{color:'#2A2520',fontSize:15,margin:'0 0 16px'}}>Dear Sarah,</p>
                  <p style={{color:'#4A4540',fontSize:14,lineHeight:1.6,margin:'0 0 24px'}}>A friendly reminder that the following invoice is due. Please don't hesitate to reach out with any questions.</p>

                  {/* Invoice summary box */}
                  <div style={{background:'#F7F3EE',border:'1px solid #E8E0D5',borderRadius:4,padding:'20px 24px',margin:'0 0 24px'}}>
                    <table style={{width:'100%',borderCollapse:'collapse'}}>
                      <tbody>
                        <tr>
                          <td style={{color:'#8A8278',fontFamily:'monospace',fontSize:11,textTransform:'uppercase',padding:'4px 0'}}>Invoice</td>
                          <td style={{color:'#2A2520',fontSize:14,fontWeight:'bold',textAlign:'right',padding:'4px 0'}}>INV-2041</td>
                        </tr>
                        <tr>
                          <td style={{color:'#8A8278',fontFamily:'monospace',fontSize:11,textTransform:'uppercase',padding:'4px 0'}}>Invoice Total</td>
                          <td style={{color:'#2A2520',fontSize:14,textAlign:'right',padding:'4px 0'}}>$6,800.00</td>
                        </tr>
                        <tr>
                          <td style={{color:'#8A8278',fontFamily:'monospace',fontSize:11,textTransform:'uppercase',padding:'4px 0'}}>Payments Received</td>
                          <td style={{color:'#6B7C6E',fontSize:14,textAlign:'right',padding:'4px 0'}}>-$2,500.00</td>
                        </tr>
                        <tr style={{borderTop:'1px solid #E8E0D5'}}>
                          <td style={{color:'#C4622D',fontFamily:'monospace',fontSize:11,textTransform:'uppercase',padding:'8px 0 4px',fontWeight:'bold'}}>Balance Due</td>
                          <td style={{color:'#C4622D',fontSize:18,fontWeight:'bold',textAlign:'right',padding:'8px 0 4px'}}>$4,300.00</td>
                        </tr>
                        <tr>
                          <td style={{color:'#8A8278',fontFamily:'monospace',fontSize:11,textTransform:'uppercase',padding:'4px 0'}}>Due Date</td>
                          <td style={{color:'#2A2520',fontSize:14,textAlign:'right',padding:'4px 0'}}>April 15, 2026</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>

                  <p style={{color:'#4A4540',fontSize:14,lineHeight:1.6,margin:'0 0 32px'}}>{thankYou}</p>
                  <p style={{color:'#8A8278',fontSize:11,fontFamily:'monospace',margin:0,paddingTop:16,borderTop:'1px solid #E8E0D5'}}>
                    {studioName}{form.website ? ` · ${form.website}` : ''}
                  </p>
                </div>
              </div>
            </div>

            <div style={{padding:'1rem 1.5rem',background:'#F7F3EE',borderTop:'1px solid rgba(42,37,32,0.08)',display:'flex',justifyContent:'flex-end'}}>
              <button onClick={() => setShowPortalPreview(false)}
                style={{padding:'0.5rem 1.25rem',fontSize:'0.82rem',borderRadius:4,border:'none',background:'#C4622D',color:'white',cursor:'pointer',fontWeight:500}}>
                Close Preview
              </button>
            </div>
          </div>
...
// ── AI EMAIL COMPOSER ──────────────────────────────────────
function AIEmailComposer({ client, studioProfile, onUseEmail, onClose }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(42,37,32,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:'1rem'}}>
      <div style={{background:'#FDFAF6',borderRadius:12,boxShadow:'0 20px 60px rgba(42,37,32,0.25)',width:'100%',maxWidth:560,overflow:'hidden'}}>
        <div style={{background:'#2A2520',padding:'1.25rem 1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:'0.6rem'}}>
            <Wand2 size={16} style={{color:'#B8963E'}}/>
            <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.1rem',color:'#F7F3EE'}}>AI Email Composer</span>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#8A8278'}}><X size={18}/></button>
        </div>
        <div style={{padding:'1.5rem'}}>
          <p style={{fontSize:'0.82rem',color:'#8A8278',marginBottom:'1rem'}}>
            Composing for: <span style={{color:'#2A2520',fontWeight:500}}>{client.name}</span>
          </p>
          <Field label="What do you want to say? (2–3 sentences is fine)">
            <textarea value={intent} onChange={e => setIntent(e.target.value)} rows={3}
              placeholder="e.g. Follow up after today's site visit, confirm the sofa fabric is approved, and let her know the lead time is 10 weeks"
              className={inputClass} style={{...inputStyle,resize:'vertical'}} />
          </Field>
          <button onClick={handleGenerate} disabled={!intent.trim() || loading}
            style={{marginTop:'0.75rem',background:intent.trim()&&!loading?'#C4622D':'#C4B5A0',color:'white',padding:'0.5rem 1.25rem',borderRadius:4,fontSize:'0.82rem',fontWeight:500,border:'none',cursor:intent.trim()?'pointer':'not-allowed',display:'flex',alignItems:'center',gap:'0.5rem'}}>
            {loading ? <><Loader size={14} className="animate-spin"/> Composing…</> : <><Wand2 size={14}/> Compose Email</>}
          </button>

          {error && <p style={{color:'#C4622D',fontSize:'0.78rem',marginTop:'0.75rem'}}>{error}</p>}

          {result && (
            <div style={{marginTop:'1.25rem'}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'0.58rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'#8A8278',marginBottom:'0.5rem'}}>Generated Email</div>
              <div style={{background:'#F7F3EE',border:'1px solid rgba(42,37,32,0.1)',borderRadius:6,padding:'1rem',fontSize:'0.85rem',color:'#2A2520',lineHeight:1.7,whiteSpace:'pre-wrap',maxHeight:220,overflowY:'auto'}}>
                {result}
              </div>
              <div style={{display:'flex',gap:'0.75rem',marginTop:'0.75rem',justifyContent:'flex-end'}}>
                <button onClick={handleGenerate} disabled={loading}
                  style={{padding:'0.45rem 1rem',fontSize:'0.78rem',border:'1px solid rgba(42,37,32,0.15)',borderRadius:4,background:'transparent',color:'#4A4540',cursor:'pointer'}}>
                  Regenerate
                </button>
                <button onClick={() => onUseEmail(result)}
                  style={{padding:'0.45rem 1.1rem',fontSize:'0.78rem',borderRadius:4,border:'none',background:'#6B7C6E',color:'white',cursor:'pointer',fontWeight:500}}>
                  Use This Email →
                </button>
              </div>
            </div>
...
// ── AI PROJECT BRIEF ───────────────────────────────────────
function AIProjectBrief({ client, projects, studioProfile, onClose }) {
  ...
  async function handleGenerate() {
    ...
    setResult('')
    try {
      const context = clientProjects.length > 0
        ? `Existing projects: ${clientProjects.map(p => p.name).join(', ')}.`
        : 'No existing projects yet.'
      const text = await callClaude(
        `You are a senior interior designer at ${studioName}. Write clear, professional project briefs for new design engagements. Format as: PROJECT SCOPE (2-3 sentences), DESIGN DIRECTION (style, palette, mood — 2-3 sentences), BUDGET ESTIMATE (provide a realistic range), TIMELINE (realistic phases with weeks), KEY CONSIDERATIONS (3-4 bullet points of important factors). Be specific and actionable. Use professional interior design terminology.`,
        `Client: ${client.name}. ${context} Project description: ${description.trim()}`
      )
      setResult(text.trim())
    } catch(e) {
      setError('Could not generate brief. Please check your API connection.')
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(42,37,32,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:'1rem',overflowY:'auto'}}>
      <div style={{background:'#FDFAF6',borderRadius:12,boxShadow:'0 20px 60px rgba(42,37,32,0.25)',width:'100%',maxWidth:620,overflow:'hidden',margin:'auto'}}>
        <div style={{background:'#2A2520',padding:'1.25rem 1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:'0.6rem'}}>
            <Wand2 size={16} style={{color:'#B8963E'}}/>
            <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.1rem',color:'#F7F3EE'}}>AI Project Brief Generator</span>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#8A8278'}}><X size={18}/></button>
        </div>
        <div style={{padding:'1.5rem'}}>
          <p style={{fontSize:'0.82rem',color:'#8A8278',marginBottom:'1rem'}}>
            Client: <span style={{color:'#2A2520',fontWeight:500}}>{client.name}</span>
          </p>
          <Field label="Describe the project in your own words">
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={4}
              placeholder="e.g. Full living room renovation in their Upper East Side prewar apartment. Classic with a modern edge. They love warm neutrals, quality materials, and good lighting. Budget is flexible, probably $60-80K. They want it done before Thanksgiving."
              className={inputClass} style={{...inputStyle,resize:'vertical'}} />
          </Field>
          <button onClick={handleGenerate} disabled={!description.trim() || loading}
            style={{marginTop:'0.75rem',background:description.trim()&&!loading?'#C4622D':'#C4B5A0',color:'white',padding:'0.5rem 1.25rem',borderRadius:4,fontSize:'0.82rem',fontWeight:500,border:'none',cursor:description.trim()?'pointer':'not-allowed',display:'flex',alignItems:'center',gap:'0.5rem'}}>
            {loading ? <><Loader size={14} className="animate-spin"/> Generating Brief…</> : <><Wand2 size={14}/> Generate Project Brief</>}
          </button>
          ...
          {result && (
            <div style={{marginTop:'1.25rem'}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'0.58rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'#8A8278',marginBottom:'0.5rem'}}>Generated Project Brief</div>
              <div style={{background:'#F7F3EE',border:'1px solid rgba(42,37,32,0.1)',borderRadius:6,padding:'1rem',fontSize:'0.83rem',color:'#2A2520',lineHeight:1.75,whiteSpace:'pre-wrap',maxHeight:320,overflowY:'auto'}}>
                {result}
              </div>
              <div style={{display:'flex',gap:'0.75rem',marginTop:'0.75rem',justifyContent:'flex-end'}}>
                <button onClick={handleGenerate} disabled={loading}
...
function AIProcurementAssistant({ client, project, studioProfile, onClose }) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const text = await callClaude(
        `You are a senior interior designer at ${studioName} with deep knowledge of luxury trade vendors (RH, Visual Comfort, Holly Hunt, Arteriors, Waterworks, Kravet, Schumacher, McGee & Co, West Elm Trade, Arteriors, Baker, Brunschwig & Fils, etc.). Create curated procurement lists with specific product suggestions, realistic trade pricing, and vendor names. Format as a clean list grouped by category (Seating, Tables, Lighting, Textiles, Accessories, etc.) with: item name, suggested vendor, approximate trade cost, and a brief note on why it fits. Include 8-12 items. Be specific — name actual product lines where possible.`,
        `${context}Client: ${client?.name || 'Client'}. Room description: ${roomDesc.trim()}`
      )
      setResult(text.trim())
    } catch(e) {
      setError('Could not generate suggestions. Please check your API connection.')
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(42,37,32,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:'1rem',overflowY:'auto'}}>
      <div style={{background:'#FDFAF6',borderRadius:12,boxShadow:'0 20px 60px rgba(42,37,32,0.25)',width:'100%',maxWidth:640,overflow:'hidden',margin:'auto'}}>
        <div style={{background:'#2A2520',padding:'1.25rem 1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:'0.6rem'}}>
            <Wand2 size={16} style={{color:'#B8963E'}}/>
            <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.1rem',color:'#F7F3EE'}}>AI Procurement Assistant</span>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#8A8278'}}><X size={18}/></button>
        </div>
        <div style={{padding:'1.5rem'}}>
          {client && <p style={{fontSize:'0.82rem',color:'#8A8278',marginBottom:'1rem'}}>
            {project ? <>Project: <span style={{color:'#2A2520',fontWeight:500}}>{project.name}</span></> : <>Client: <span style={{color:'#2A2520',fontWeight:500}}>{client.name}</span></>}
          </p>}
          <Field label="Describe the room and design direction">
            <textarea value={roomDesc} onChange={e => setRoomDesc(e.target.value)} rows={4}
              placeholder="e.g. Primary bedroom suite, approx 600sf. Warm transitional style — cream and camel tones, natural textures, brass accents. Need: king bed, two nightstands, lounge chair, drapery, chandelier, bedside sconces, area rug. Quality level: luxury trade. Client appreciates investment pieces."
              className={inputClass} style={{...inputStyle,resize:'vertical'}} />
          </Field>
          <button onClick={handleGenerate} disabled={!roomDesc.trim() || loading}
            style={{marginTop:'0.75rem',background:roomDesc.trim()&&!loading?'#C4622D':'#C4B5A0',color:'white',padding:'0.5rem 1.25rem',borderRadius:4,fontSize:'0.82rem',fontWeight:500,border:'none',cursor:roomDesc.trim()?'pointer':'not-allowed',display:'flex',alignItems:'center',gap:'0.5rem'}}>
            {loading ? <><Loader size={14} className="animate-spin"/> Sourcing suggestions…</> : <><Wand2 size={14}/> Generate Procurement List</>}
          </button>
          <p style={{fontSize:'0.72rem',color:'#C4B5A0',marginTop:'0.4rem'}}>Takes 10–20 seconds — Claude is thinking through vendors and products</p>

          {error && <p style={{color:'#C4622D',fontSize:'0.78rem',marginTop:'0.75rem'}}>{error}</p>}

          {result && (
            <div style={{marginTop:'1.25rem'}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'0.58rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'#8A8278',marginBottom:'0.5rem'}}>Suggested Procurement List</div>
              <div style={{background:'#F7F3EE',border:'1px solid rgba(42,37,32,0.1)',borderRadius:6,padding:'1rem',fontSize:'0.82rem',color:'#2A2520',lineHeight:1.8,whiteSpace:'pre-wrap',maxHeight:360,overflowY:'auto'}}>
                {result}
              </div>
              <div style={{display:'flex',gap:'0.75rem',marginTop:'0.75rem',justifyContent:'flex-end'}}>
                <button onClick={handleGenerate} disabled={loading}
