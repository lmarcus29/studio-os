    ...
    <div style={{minHeight:'100vh',background:'#F7F3EE',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'#FDFAF6',borderRadius:8,boxShadow:'0 8px 40px rgba(42,37,32,0.12)',padding:'2rem',width:'100%',maxWidth:360}}>
        <div className="text-center mb-8">
          <h1 style={{fontFamily:"'Cormorant Garamond', serif",fontSize:'1.8rem',fontWeight:300,color:'#2A2520',letterSpacing:'0.02em'}}>Elegant Interiors</h1>
          <p style={{fontSize:'0.78rem',color:'#8A8278',marginTop:'0.25rem',fontFamily:"'DM Mono', monospace",letterSpacing:'0.1em',textTransform:'uppercase',fontSize:'0.65rem'}}>Interior Design Studio</p>
        </div>
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(42,37,32,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:'1rem'}}>
      <div style={{background:'#FDFAF6',borderRadius:8,boxShadow:'0 8px 40px rgba(42,37,32,0.15)',padding:'1.5rem',width:'100%',maxWidth:600,maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.75rem'}}>
          ...
          <Field label="Product URL"><input value={form.url||''} onChange={e => set('url', e.target.value)} placeholder="https://www.vendor.com/product" className={inputClass} style={inputStyle}/></Field>
...
function Items({ items, projects, vendors, clients, reload }) {
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState('All')
  ...
  return (
    <div>
                ...
                <tr key={i.id} style={{borderBottom:'1px solid rgba(42,37,32,0.04)'}}>
                  ...
                  <td style={{padding:'0.75rem 1.25rem'}}>
                    <div style={{display:'flex',gap:'0.4rem',alignItems:'center'}}>
                      ...
                      <Actions onEdit={() => setModal(i)} onDelete={() => setDeleteTarget(i)}/>
                    </div>
                  </td>
                </tr>
              )
      ...
      {modal && <ItemModal item={modal === 'add' ? null : modal} projects={projects} vendors={vendors} clients={clients} onSave={handleSave} onClose={() => setModal(null)} />}
      {deleteTarget && <ConfirmDeleteModal name={deleteTarget.name} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} loading={deleteLoading} />}
    </div>
  )
}
...
async function handleDelete() {
  ...
  function generatePDF(invoice) {
    ...
    const brand = {
      ...
      website: 'www.elegantinteriors.com',
      email: 'studio@elegantinteriors.com',
      phone: '(212) 555-0199',
      address: '100 Design District · New York, NY 10001',
...
function CalendarView({ events, reload, clients }) {
  ...
  const btnStyle = (active) => ({
    ...
    background: active ? '#2A2520' : 'transparent',color: active ? 'white' : '#4A4540',cursor:'pointer',fontFamily:"'DM Mono',monospace",letterSpacing:'0.05em'
  })
      ...
      <div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
            ...
            return (
              <div key={day} style={{minHeight:72,padding:6,border:`1px solid ${isToday ? '#C4622D' : 'rgba(42,37,32,0.08)'}`,borderRadius:6,background: isToday ? '#F5E6DE' : 'transparent',cursor:'pointer'}}
                ...
                {dayEvents.slice(0,3).map(e => (
                  <div key={e.id} onClick={ev => { ev.stopPropagation(); setModal(e) }}
                    style={{...(typeColors[e.type]||{background:'#E8E0D5',color:'#8A8278'}),fontSize:'0.62rem',borderRadius:3,padding:'0.1rem 0.3rem',marginBottom:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',cursor:'pointer'}}>
    ...
    return (
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>
        {days.map(d => {
          ...
          const dayEvents = eventsOnDate(dateStr)
          return (
            <div key={dateStr} style={{minHeight:200,border:`1px solid ${isToday ? '#C4622D' : 'rgba(42,37,32,0.08)'}`,borderRadius:6,overflow:'hidden',background: isToday ? '#F5E6DE' : '#FDFAF6'}}>
              <div style={{padding:'0.5rem',borderBottom:'1px solid rgba(42,37,32,0.06)',textAlign:'center'}}>
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.2rem',color: isToday ? '#C4622D' : '#2A2520'}}>{d.getDate()}</div>
              </div>
              <div style={{padding:'0.4rem'}}>
                {dayEvents.map(e => (
                  <div key={e.id} onClick={() => setModal(e)}
                    style={{...(typeColors[e.type]||{background:'#E8E0D5',color:'#8A8278'}),fontSize:'0.7rem',borderRadius:4,padding:'0.25rem 0.4rem',marginBottom:3,cursor:'pointer'}}>
                    {e.time && <div style={{fontFamily:"'DM Mono',monospace",fontSize:'0.6rem',opacity:0.8}}>{e.time}</div>}
  ...
  function DayView() {
    ...
    return (
      <div>
        ...
        {dayEvents.length === 0
          ? <p style={{textAlign:'center',color:'#8A8278',fontSize:'0.82rem',padding:'2rem 0'}}>No events scheduled</p>
          : dayEvents.map(e => (
            <div key={e.id} onClick={() => setModal(e)}
              style={{display:'flex',gap:'1rem',padding:'0.75rem 1rem',marginBottom:'0.5rem',borderRadius:6,...(typeColors[e.type]||{background:'#E8E0D5',color:'#8A8278'})}}>
              <div style={{flexShrink:0,width:60,textAlign:'right'}}>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'0.72rem',fontWeight:500}}>{e.time || '—'}</div>
                {e.duration && <div style={{fontSize:'0.65rem',opacity:0.7}}>{e.duration}m</div>}
              </div>
              <div style={{flex:1}}>
                <div style={{fontWeight:500,fontSize:'0.9rem',marginBottom:2}}>{e.title}</div>
                {e.location && <div style={{fontSize:'0.75rem',opacity:0.8}}>📍 {e.location}</div>}
                {e.notes && <div style={{fontSize:'0.72rem',opacity:0.7,marginTop:2}}>{e.notes}</div>}
              </div>
              <div style={{display:'flex',gap:'0.4rem',flexShrink:0,alignItems:'flex-start'}}>
                <button onClick={ev => {ev.stopPropagation();setModal(e)}} style={{background:'none',border:'none',cursor:'pointer',opacity:0.7,display:'flex'}}><Pencil size={13}/></button>
                <button onClick={ev => {ev.stopPropagation();setDeleteTarget(e)}} style={{background:'none',border:'none',cursor:'pointer',opacity:0.7,display:'flex'}}><Trash2 size={13}/></button>
              </div>
            </div>
          ))
        }
