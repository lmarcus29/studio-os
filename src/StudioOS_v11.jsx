  ...
  return (
    <div>
      ...
      <div style={{marginBottom:'1.5rem',display:'flex',alignItems:'flex-end',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:"'DM Mono',monospace",fontSize:'0.6rem',color:'#C4B5A0',letterSpacing:'0.1em'}}>v11</span>
      </div>
...
function Files({ projects, clients, fileMetadata, reload }) {
  ...
  async function handleDelete() {
    setDeleteLoading(true)
    // Always delete metadata — don't block on storage error (handles orphan files)
    try { await supabase.storage.from('studio-files').remove([deleteTarget.storagePath]) } catch(e) {}
    if (deleteTarget.metaId) {
      await supabase.from('file_metadata').delete().eq('id', deleteTarget.metaId)
    } else {
      // File from storage with no metadata record — just reload
    }
    setDeleteLoading(false)
    setDeleteTarget(null)
    reload()
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem'}}>
          ...
          {filtered.map(f => (
            <div key={f.name} style={{background:'#FDFAF6',border:'1px solid rgba(42,37,32,0.1)',borderRadius:8,overflow:'hidden'}}>
              <div style={{height:140,background:'#E8E0D5',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',cursor: isImage(f.name||f.displayName) ? 'zoom-in' : 'default'}}
              ...
              <div style={{padding:'0.75rem'}}>
                ...
                <div style={{display:'flex',gap:'0.5rem',marginTop:'0.5rem',alignItems:'center'}}>
                  ...
                  <button onClick={() => setDeleteTarget(f)} style={{display:'flex',alignItems:'center',gap:3,fontSize:'0.72rem',color:'#C4B5A0',background:'none',border:'none',cursor:'pointer',marginLeft:'auto'}}><Trash2 size={12}/> Delete</button>
                </div>
              </div>
            </div>
          ))}
